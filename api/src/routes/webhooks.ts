...
webhookRouter.get('/:agentId/logs', (req, res) => {
  ...
  const recentLogs = logs.sort((a, b) => b.timestamp - a.timestamp).slice(0, 50); // Last 50 deliveries
...
webhookRouter.patch(
  '/:agentId/:webhookId/status',
  [body('active').isBoolean().withMessage('Active must be boolean'), validate],
  (req: Request, res: Response) => {
...
export async function triggerWebhook(event: WebhookEvent, payload: any, agentId?: string) {
  ...
  if (agentId && webhooks.has(agentId)) {
    webhooks.get(agentId)!.forEach(sub => {
      if (sub.active && sub.events.includes(event) && !allWebhooks.find(w => w.sub.id === sub.id)) {
