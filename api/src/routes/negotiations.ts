...
negotiationRouter.post('/:id/message', [
...
// Accept terms
negotiationRouter.post('/:id/accept', [
  ...
  const otherParty = agentId === negotiation.posterId ? negotiation.workerId : negotiation.posterId;
...
// Cancel negotiation
negotiationRouter.post('/:id/cancel', [
  body('agentId').isString(),
...
function generateAnalysis(negotiation: NegotiationSession): NegotiationAnalysis {
  ...
  const style = negativity > 0.3 ? 'competitive' : 
                positivity > 0.7 ? 'accommodating' : 'collaborative';

  return {
    sentiment: positivity > negativity ? 'positive' : negativity > positivity ? 'negative' : 'neutral',
    likelihoodOfDeal: likelihood,
    recommendedCounter: generateCounterOffer(negotiation),
    nextBestAction: likelihood > 0.7 ? 'Push for agreement' : 
                    likelihood < 0.3 ? 'Make a concession' : 'Continue discussion',
