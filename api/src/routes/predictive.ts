...
predictiveRouter.post('/feedback', [
  ...
  const successes = profile.pastTasks.filter(t => t.includes('success')).length + (success ? 1 : 0);
  ...
  if (completionTime) {
    if (profile.avgCompletionTime === 0) {
      ...
      profile.avgCompletionTime = (profile.avgCompletionTime * (totalTasks - 1) + completionTime) / totalTasks;
    }
  }
...
predictiveRouter.get('/agent/:agentId/analytics', (req, res) => {
  ...
  const recentSuccessRate = recentTasks.filter(t => t.includes('success')).length / recentTasks.length;
...
function calculateMatchScore(agent: AgentProfile, task: TaskProfile): MatchScore {
  ...
  // Estimate timeline
  const predictedTimeline = agent.avgCompletionTime > 0 
...
function findBestTeam(candidates: AgentProfile[], requiredSkills: string[], size: number): AgentProfile[] {
